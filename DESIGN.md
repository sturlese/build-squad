...
A rule the model cannot break beats a rule it promises to follow. The auditor cannot modify anything — it has no write tools, not a "please don't" paragraph. The documentator cannot run builds or tests — it has no shell. Dangerous operations (`kubectl` and work-destroying git commands — `git reset --hard`, `git clean -f`, `git checkout -- .`, `git restore .`, `git push --force`) are blocked by a `PreToolUse` hook that exits before the tool runs. Prose rules remain for what tooling can't express (prompt-injection handling, secret redaction), but every boundary that can be mechanical is mechanical.
